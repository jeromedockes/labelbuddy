...
More generally underlined letters in the GUI indicate kbd:[Alt]-key shortcuts: for example "`[.underline]##N##ew label`" indicates you can jump to creating a new label by pressing kbd:[Alt+N].
...
The examples shown in this section can be found as separate files on the {homepage}[{lb} website].
...
At the moment, {lb} excepts all the files it imports to be encoded with {wikipedia-utf8}[UTF-8] (or UTF-16, or UTF-32).
...
Note that {lb} only deals with text (non-binary) formats.
...
[source,txt]
.Importing two documents from a `.txt` file
----
...
[source,json]
.Importing two documents from a `.json` file.
----
...
[source,json]
.Two documents and their annotations in a `.json` file.
----
...
For example:
[source,json]
.Importing two documents from a `.jsonl` file.
----
...
[source,json]
.Two documents and their annotations in a `.jsonl` file.
----
...
A RELAX NG schema is provided on the {homepage}[{lb} website].
...
For example:
[source,xml]
.Importing two documents from a `.xml` file.
----
...
[source,xml]
.Two documents and their annotations in a `.xml` file.
----
...
[source,csv]
.Importing two documents from a `.csv` file.
----
...
[source,csv]
.Two documents and their annotations in a `.csv` file.
----
...
For example:
[source,json]
.Importing two labels from a `.json` file
----
...
For example:
[source,json]
.Importing two labels from a `.jsonl` file.
----
...
As for documents, a RELAX NG schema is provided on the {homepage}[{lb} website].
Here is an example:
[source,xml]
.Importing two labels from a `.xml` file.
----
