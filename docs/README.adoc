...
:examples-url: https://jeromedockes.github.io/labelbuddy/example_data
:schemas-url: https://jeromedockes.github.io/labelbuddy/schema
...
ifdef::env-github[]
- Website: {homepage}[jeromedockes.github.io/labelbuddy/]
- Documentation for latest release: {homepage}documentation.html[{homepage-no-scheme}documentation.html]
endif::[]
...
The examples shown in this section can be found as separate files on the link:{examples-url}/[{lb} website].
...
A RELAX NG schema is provided on the link:{schemas-url}[{lb} website].
...
As for documents, a RELAX NG schema is provided on the link:{schemas-url}[{lb} website].
Here is an example:
[source,xml]
