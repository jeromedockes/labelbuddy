...
When importing annotations, at the moment {lb} relies on the Unicode indices `start_char` and `end_char` -- if you create annotations outside of {lb} that you want to import, you can omit `start_byte` and `end_byte` but must provide `start_char` and `end_char`.
