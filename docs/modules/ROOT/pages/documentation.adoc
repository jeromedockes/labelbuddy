...
For example if the text starts with "`hello &#x1F600;`" and you highlighted exactly that phrase, and labelled it with `label_1`, the associated annotation will be `{"start_char": 0, "end_char": 7, "label_name": "label_1"}`.
If you also typed "`some more info`" in the {extra-edit} box, the annotation will be `` {"start_char": 0, "end_char": 7, "label_name": "label_1", "extra_data": "some more info"}`.
