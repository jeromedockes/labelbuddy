...
A document can have a `annotations` attribute, which is a list of annotations.
...
For example if the text starts with "`hello &#x1F600;`" and you highlighted exactly that phrase, and annotated it with the label *"Greeting"*, the associated annotation will be `{"start_char": 0, "end_char": 7, "label_name": "Greeting"}`.
If you also typed "`some more info`" in the {extra-edit} box, the annotation will be `` {"start_char": 0, "end_char": 7, "label_name": "Greeting", "extra_data": "some more info"}`.
