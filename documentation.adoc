...
Start by <<installation.adoc#from-binary-copies,installing>> {lb}.
...
NOTE: a <<managing-projects,labelbuddy database>> is actually just a regular file on your disk (an https://www.sqlite.org/index.html[SQLite] database).
...
Each {lb} project (a set of documents, labels and annotations) is an https://www.sqlite.org/index.html[SQLite] database.
...
{lb} was created using C++, https://www.qt.io/[Qt], https://www.sqlite.org/index.html[SQLite], tools from the https://www.gnu.org/[GNU project], and more.
